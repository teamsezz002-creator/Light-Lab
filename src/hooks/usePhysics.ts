...
export function usePhysics(state: SimulationState) {
  ...
  const result = useMemo(() => {
    const rawDistance = objectDistance === 0 ? 0.001 : objectDistance;
    const isRightSide = rawDistance < 0;
    const absDistance = Math.abs(rawDistance);

    // Standard sign convention: Object is placed at -u logically
    const u = -absDistance; 
    let f = 0;
    ...
    const objPos = { x: POLE_X - (absDistance * sign), y: POLE_Y };
    const imgPos = { x: POLE_X + (v * sign), y: POLE_Y };

    // Zone Detection
    let zone = "";
    if (mode === 'mirror' && mirrorType === 'concave') {
      if (absDistance > 2*focalLength) zone = "Beyond C";
      else if (Math.abs(absDistance - 2*focalLength) < 5) zone = "At C";
      else if (absDistance > focalLength) zone = "Between C and F";
      else if (Math.abs(absDistance - focalLength) < 5) zone = "At F";
      else zone = "Between F and P";
    } else if (mode === 'lens' && lensType === 'convex') {
      if (absDistance > 2*focalLength) zone = "Beyond 2F1";
      else if (Math.abs(absDistance - 2*focalLength) < 5) zone = "At 2F1";
      else if (absDistance > focalLength) zone = "Between F1 and 2F1";
      else if (Math.abs(absDistance - focalLength) < 5) zone = "At F1";
